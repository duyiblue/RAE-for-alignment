#!/usr/bin/env python
"""
Alignment training script for cross-domain RAE encoder alignment.
...
class AlignmentTrainer(pl.LightningModule):
    ...
    def __init__(
        ...
        """
        Args:
            source_config_path: Path to source model config (RAE config YAML)
            target_config_path: Path to target model config (RAE config YAML)
        ...
        target_rae_config, *_ = parse_configs(target_config_path)
        assert target_rae_config is not None, f"No stage_1 section found in config {target_config_path}"
        self.target_model: RAE = instantiate_from_config(target_rae_config)
        ...
        source_rae_config, *_ = parse_configs(source_config_path)
        assert source_rae_config is not None, f"No stage_1 section found in config {source_config_path}"
        self.source_model: RAE = instantiate_from_config(source_rae_config)
        
        # Freeze source decoder (only train encoder)
        ...
        self.ssim_metric = StructuralSimilarityIndexMeasure(data_range=1.0)  # Images in [0, 1]
    ...
    def forward(self, batch):
        ...
        with torch.no_grad():
            target_latent = self.target_model.encode(target_image)
        
        # Encode with source model (trainable)
        source_latent = self.source_model.encode(source_image)
    ...
    def compute_loss(self, source_latent, target_latent, target_image=None):
        """
...
def save_visualization(images_dict, output_path):
    ...
    for i in range(n_samples):
        for j, (key, title) in enumerate(zip(keys, titles)):
            ...
            # Images are already in [0, 1] range for RAE
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description='Alignment training for RAE encoders')
